"""
SATOSA microservice that consumes an identifier passed to it after
the primary identifier microservice runs. The consumed identifier
is examined for two possible cases:

1) The authenticating IdP is the NYU Langone IdP and the consumed
identifier has scope nyumc.org. In this case the scope is stripped
from the identifier and the remaining part is asserted to the SP
in the attribute configured by 'nyu_langone_return_attribute'.

2) The authenticating IdP is not the NYU Langone IdP. In this case
the consumed identifier is used as a key to search an LDAP
directory for a record. If a record is found then the consumed
identifier and other found attributes are asserted to the SP 
in the attributes configured by
'search_return_attributes'. If a record is not found then the user
is redirected to an external page with further instructions.
"""
...
class NyuLangoneAttributeStoreError(SATOSAError):
    """
    NYU Langone attribute store error
    """
    pass

class NyuLangoneAttributeStore(satosa.micro_services.base.ResponseMicroService):
    """
    Consumer identifier from the primary identifier microservice and
    either process as a NYU Langone user by removing the @nyumc.org
    scope or a federated user in which case lookup a person record in 
    LDAP. If a person record is found assert the primary identifier
    or if not redirect the user.
    """
    ...
    config_defaults = {
        ...
        'ldap_url'                      : None,
        'nyu_langone_return_attribute'  : 'uid',
